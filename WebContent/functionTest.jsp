	...
	<div class="container">
		<table class="table table-striped">
			...
			<tr>
				...
				<td><a href="ProductEdit.jsp?id=<%=p.getProductID()%>">Edit</a></td>
				<td><a href="ProductDelCode.jsp?id=<%=p.getProductID()%>"
					onclick="return confirm('確認刪除');">Del</a></td>
			</tr>
			<%
				}
			%>
		...
		<p>
			...
			<input type="button" name="prev" value="上一頁"
				onclick="location.href='functionTest.jsp?p=<%=(pg - 1)%>'">
			<a href="functionTest.jsp?p=<%=(pg - 1)%>">上一頁</a>
			<%
				}
			%>
			<%
			...
			<input type="button" name="next" value="下一頁"
				onclick="location.href='functionTest.jsp?p=<%=(pg + 1)%>'">
			<a href="functionTest.jsp?p=<%=(pg + 1)%>">下一頁</a>
			<%
				}
			%>
		...
		<form id="page2" name="page2" action="functionTest.jsp" method="get">
			<select name="p" onchange="page2.submit()">
				...
				%>
				<option value="<%=i%>" <%if (pg == i) {%> selected="selected" <%}%>>第
					<%=i%> 頁
				</option>
				<%
					}
